...
<a href="{{route('makanan.admin.home')}}" class="btn btn-primary mb-3">BACK</a>
    <div class="card">
        ...
        <form action="{{route('makanan.store',$makanan->id)}}" method="Post" enctype="multipart/form-data">
                  @csrf
        ...
        <input type="text" class="form-control" name="nama_makanan" value="{{$makanan->nama_makanan}}">
            ...
            <input type="text" class="form-control" name="asal_daerah" value="{{$makanan->asal_daerah}}">
<br>
                <label for="isi">Deskripsi Makanan</label>
            <textarea id="" cols="30" rows="10" class="form-control" name="deskripsi" >{{$makanan->deskripsi}}</textarea>
                <br>
